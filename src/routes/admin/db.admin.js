...
export async function GetData() {
  try {
    ...
    const langs = await sql`
        SELECT * FROM langs ORDER BY lang ASC
    `;

    return { arkans, levels, types, langs };
  } catch (ex) {
    console.log(ex);
  }
}
...
export async function SaveData(par) {
  try {
    const res = await sql`
      INSERT INTO text
			(data, arkan , level, type, lang)
			VALUES(${par.text},${par.arkan},${par.level},${par.type}, ${par.lang})
			ON CONFLICT (arkan, level, type, lang)
			DO UPDATE SET
			data = EXCLUDED.data`;
    return res;
  } catch (ex) {
